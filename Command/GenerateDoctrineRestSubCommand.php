...
use IsmaAmbrosi\Bundle\GeneratorBundle\Generator\DoctrineRestSubGenerator;
...
use IsmaAmbrosi\Bundle\GeneratorBundle\Manipulator\RestSubRoutingManipulator;
...
class GenerateDoctrineRestSubCommand extends GenerateDoctrineCommand
{
    ...
    protected function configure()
    {
        $this
            ->setDefinition(array(
                ...
                new InputOption('subdocument', '', InputOption::VALUE_REQUIRED, 'The subdocument class name to initialize (shortcut notation)'),
...
EOT
            ...
            ->setName('doctrine:mongodb:generate:rest:sub')
            ->setAliases(array('generate:doctrine:mongodb:rest:sub'));
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if ('annotation' != $format) {
            call_user_func($runner, $this->updateRouting($dialog, $input, $output, $bundle, $format, $document, $subdocument, $prefix));
        }
    ...
    protected function interact(InputInterface $input, OutputInterface $output)
    {
        ...
        list($document, $subdocument,$bundle) = $this->askForDocument($input, $output, $dialog);
    ...
    protected function getGenerator( )
    {
        if (null === $this->generator) {
            $this->generator = new DoctrineRestSubGenerator( $this->getFilesystem(), __DIR__.'/../Resources/skeleton/subrest');
        }
    ...
    private function askForDocument(InputInterface $input, OutputInterface $output, DialogHelper $dialog)
    {
        ...
        $subdocument = $dialog->askAndValidate($output, $dialog->getQuestion('The Sub-document shortcut name', $input->getOption('subdocument')), array(
            'IsmaAmbrosi\Bundle\GeneratorBundle\Command\Validators',
            'validateDocumentName'
        ), false, $input->getOption('subdocument'));
        $input->setOption('subdocument', $subdocument);
        list($bundle, $subdocument) = $this->parseShortcutNotation($subdocument);

        return array($document, $subdocument, $bundle);
    }
     ...
     * @return array|null
     */
    private function updateRouting(DialogHelper $dialog, InputInterface $input, OutputInterface $output, BundleInterface $bundle, $format, $document, $subdocument, $prefix)
    {
        $auto = true;
        if ($input->isInteractive()) {
        ...
        $routing = new RestSubRoutingManipulator($bundle->getPath().'/Resources/config/routing/'.strtolower($document).'_routing.yml');
        
        $ret     = $auto ? $routing->addResource($bundle->getNamespace(), $document, $subdocument, $format, $document.'/'.$document.'Controller', $subdocument.'Controller', true) : false;
        if (!$ret) {
